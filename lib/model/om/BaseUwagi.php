...
/**
 * Base class that represents a row from the 'uwagi' table.
 *
 * 
 *
 ...
 */
abstract class BaseUwagi extends BaseObject  implements Persistent {
	 ...
	 * @return     Uwagi The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = UwagiPeer::ID;
		}
	...
	public function setPupil($v)
	{
		...
		if ($this->pupil !== $v) {
			$this->pupil = $v;
			$this->modifiedColumns[] = UwagiPeer::PUPIL;
		}
	...
	public function setLekcjaId($v)
	{
		...
		if ($this->lekcja_id !== $v) {
			$this->lekcja_id = $v;
			$this->modifiedColumns[] = UwagiPeer::LEKCJA_ID;
		}
	...
	public function setInfo($v)
	{
		...
		if ($this->info !== $v) {
			$this->info = $v;
			$this->modifiedColumns[] = UwagiPeer::INFO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = UwagiPeer::NUM_COLUMNS - UwagiPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Uwagi object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UwagiPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseUwagi:delete:pre') as $callable)
			{
			...
			if ($ret) {
				UwagiPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUwagi:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseUwagi:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseUwagi:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				UwagiPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = UwagiPeer::doInsert($this, $con);
					...
					$affectedRows += UwagiPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = UwagiPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = UwagiPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = UwagiPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UwagiPeer::ID)) $criteria->add(UwagiPeer::ID, $this->id);
		if ($this->isColumnModified(UwagiPeer::PUPIL)) $criteria->add(UwagiPeer::PUPIL, $this->pupil);
		if ($this->isColumnModified(UwagiPeer::LEKCJA_ID)) $criteria->add(UwagiPeer::LEKCJA_ID, $this->lekcja_id);
		if ($this->isColumnModified(UwagiPeer::INFO)) $criteria->add(UwagiPeer::INFO, $this->info);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(UwagiPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Uwagi (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Uwagi Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getUzytkownik(PropelPDO $con = null)
	{
		if ($this->aUzytkownik === null && ($this->pupil !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aUzytkownik->addUwagis($this);
			 */
		}
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseUwagi:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseUwagi::%s', $method));
	  }
